...
const Register = () => {
    ...
    const { register, isLoading, error, answer } = useAuth();
    ...
    return (
        <>
            <Header />
            <section className='min-h-screen flex items-center justify-center relative md:overflow-auto overflow-hidden'>
                ...
                <div className="flex flex-col items-center w-full max-w-[320px] md:max-w-168.25 gap-4 md:gap-12.5 relative">
                    <svg className='md:w-13 md:h-12.5 w-8 h-7.5' width="52" height="50" viewBox="0 0 52 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="p-4 md:p-8 rounded-xl md:rounded-3xl border border-(--dark) backdrop-blur-xs bg-white/10 w-full">
                        {error && (
                            <div className="bg-red-100 text-red-700 p-3 rounded mb-4">
                                {/* @ts-ignore */}
                                {error instanceof Error ? answer.error.response.data.message + " " ? answer.error.response.data.message + " " : error.message + " " : 'Ошибка регистрации'}
                            </div>
                        )}

                        <form onSubmit={handleSubmit}>
                            <div className="mb-4 md:mb-8.75">
                                <label className="block text-sm mb-.5 md:mb-1.5">Имя/Компания</label>
                                <input
                                    type="text"
                                    value={form.name}
                            ...
                            <div className="mb-4 md:mb-8.75">
                                <label className="block text-sm mb-.5 md:mb-1.5">Логин</label>
                                <input
                                    type="text"
                                    value={form.login}
                            ...
                            <div className="mb-4 md:mb-8.75">
                                <label className="block text-sm mb-.5 md:mb-1.5">Пароль</label>
                                <input
                                    type="password"
                                    value={form.password}
                            ...
                            <div className="flex items-center gap-1 mb-2 md:mb-7">
                                <input checked={agree} onChange={(e) => setAgree(e.target.checked)} type="checkbox" name="checkbox" id="checkbox" className='md:w-6 md:h-6 w-4 h-4 border border-(--dark) rounded-md bg-none' />
                                <span className='md:text-base text-xs md:m-0 ml-1 mb-1'>Я принимаю условия <NavLink to="/policy"><u>соглашения</u></NavLink></span>
                            </div>
                            ...
                            <div className="relative overflow-hidden rounded-lg md:rounded-[10px]">
                                ...
                                <button
                                    ...
                                    className="relative w-full bg-(--white) druk text-(--dark) py-2 text-base md:text-xl h-10 md:h-12.75 hover:bg-blue-100 disabled:opacity-50 rounded-lg md:rounded-[10px] [box-shadow:0px_26.2552px_46.7671px_rgba(9,14,34,0.25),inset_-13.1276px_1.64095px_21.3324px_#334E6C]"
                                >
                                    {isLoading ? 'Загрузка...' : 'Присоединиться'}
                                </button>
                            </div>
                            <span className='text-sm mt-4 flex md:flex-row flex-col w-full items-center justify-center'>Уже есть аккаунт?&nbsp;<NavLink to="/login"><u>Войти</u></NavLink></span>
                        </form>
                    </div>
                </div>
            </section>
            ...
            <SEO
                title="Регистрация"
                description="Создание аккаунта в системе управления безопасностью"
